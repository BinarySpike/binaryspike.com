<template>
  <div class="wrapper">
    <div class="listing">
      <div class="header">
        <div class="accept" @click="acceptItem"><fa-icon :icon="['fas', 'check']" /></div>
        <div :class="['title', itemTitleColorClass(item)]" :title="item.name">{{ item.name }}</div>
        <div class="delete" @click="deleteItem"><fa-icon :icon="['fas', 'trash']" /></div>
      </div>
      <div class="content"></div>
      <div class="tags" v-if="item.tags && item.tags.length"><fa-icon
          class="icon"
          v-for="tag in item.tags"
          :key="tag"
          :icon="$store.state.tags[tag].iconClass.split(' fa-')"
        /></div>
      <div v-else><fa-icon class="invisible" :icon="['fas','eye-slash']" /></div>
    </div>
  </div>
</template>
...
.listing {
  @apply flex flex-col shadow rounded-lg overflow-hidden h-full;
}
.header {
  @apply flex flex-row;
}
...
.title {
  @apply py-2 flex-auto truncate px-2 text-white font-semibold;
}
.content {
...
svg {
  @apply align-middle m-1 text-2xl ;
}
</style>
